...
const D_COEFFICIENTS = COEFFICIENTS.slice(0, 11).map((row, i) =>
  row.map(v => v * (11 - i))
);
 ...
 *
 * @param massFraction - the mass fraction of ethanol in solution (should be between 0 and 1, inclusive, to be valid)
 ...
 */
export function density(massFraction: number, temperature: number): number {
  return polyEval(
    COEFFICIENTS.map(row => polyEval(row, temperature - 20)),
    massFraction
  );
}
 ...
 * with respect to the mass fraction of the solution
 *
...
export function dDensity(p: number, t: number): number {
  return polyEval(
    D_COEFFICIENTS.map(row => polyEval(row, t - 20)),
    p
  );
}
